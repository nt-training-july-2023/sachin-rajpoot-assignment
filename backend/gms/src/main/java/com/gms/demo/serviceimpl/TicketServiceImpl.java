...
public class TicketServiceImpl implements TicketService {
  ...
  @Override
  public List<TicketGetAllOutDto> getAllTicketAuth(Integer memberId, boolean myTickets, 
      Status filter,
...
//        Page<Ticket> pageTickets = 
        Page<Ticket> tickets = ticketRepo.findAllByMemberIdOrderByStatusAsc(member.getMemberId(),
      ...
      if (filter.toString().equals("ALL")) {
        Page<Ticket> pagetickets = this.ticketRepo.findAllByDepartmentId(member.getDepartment()
            .getDepartmentId(),
            PageRequest.of(pageNumber, 8, Sort.by("status")));
        List<Ticket> tickets = pagetickets.getContent();
        List<TicketGetAllOutDto> ticketOutDtos = new ArrayList<>();
      ...
      Page<Ticket> tickets = this.ticketRepo.findAllByDepartmentIdAndStatus(member.getDepartment().
          getDepartmentId(),
          filter, PageRequest.of(pageNumber, 8, Sort.by("status")));
      List<TicketGetAllOutDto> ticketOutDtos = new ArrayList<>();
      tickets.forEach(t -> ticketOutDtos.add(this.modelMapper.map(t, TicketGetAllOutDto.class)));
