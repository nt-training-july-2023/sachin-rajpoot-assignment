...
public class DepartmentServiceImpl implements DepartmentService {
	 ...
	 */
	@Override
	public List<DepartmentOutDto> getAllDepartment(Integer pageNumber) {
		Pageable pageable = PageRequest.of(pageNumber, 6);
		Page<Department> departments = this.departmentRepo
				.findAll(PageRequest.of(pageNumber, 6, Sort.by("departmentName")));
		List<DepartmentOutDto> departmentDtos = new ArrayList<>();
		departments.forEach(department -> departmentDtos.add(this.modelMapper.map(department, DepartmentOutDto.class)));
	 ...
	 */
	@Override
	public DepartmentOutDto createDepartment2(final DepartmentDto departmentDto, final String email,
...
//    	String decodedPassword = new String(Base64.getDecoder().decode(member.getPassword()));
			if (member.getPassword().equals(password) && member.getRole() != null
				...
				return this.modelMapper.map(this.departmentRepo.save(department), DepartmentOutDto.class);
			}
		}
		return null;
	}
	...
	@Override
	public List<DepartmentOutDto> getAllDepartmentNoPage() {
